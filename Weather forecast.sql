...
select * from weather_data_cleaned;

select `Minimum pressure` from weather_data_cleaned;
...
select Date,temperature,(case when temperature<(select avg(Temperature) from weather_data_cleaned) then 'cold' else 'hot' end)
as 'hot/cold'
from weather_data_cleaned;
...
from weather_data_cleaned group by Date)
...
select date,avg(`Average humidity (%`) from weather_data_cleaned 
group by date
order by date;
...
select date,avg(`Average windspeed (mph`) from weather_data_cleaned 
group by date;
...
from weather_data_cleaned where `Maximum gust speed (mph`>=55
;


-- 7. Find the number of days when the temperature went below 0 degrees Celsius 

select count(date) from   weather_data_cleaned where `Minimum temperature (Â°F`<32 ;
